						...
						<label class="col-sm-1 control-label" for="productKindU">상품 목록</label>
							<div class="col-sm-3">	
								<select class="form-control" id="productKindU" name="productKindU" onchange="productKindChange(this)">
									...
									<option value="l" <c:if test="${searchType == 'l'}">selected</c:if>>Living</option>
									<option value="b" <c:if test="${searchType == 'b'}">selected</c:if>>Bed</option>
									<option value="c1" <c:if test="${searchType == 'c1'}">selected</c:if>>Bath</option>
								</select>
						</div>
